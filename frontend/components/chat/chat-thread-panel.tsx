...
export function ChatThreadPanel({
  ...
  const [peerStatus, setPeerStatus] = React.useState<string | undefined>(initialPeerStatus)
  const [peerLastSeenAt, setPeerLastSeenAt] = React.useState<string | undefined>(initialPeerLastSeenAt)

  React.useEffect(() => {
    setPeerStatus(initialPeerStatus)
    setPeerLastSeenAt(initialPeerLastSeenAt)
  }, [initialPeerStatus, initialPeerLastSeenAt])
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const files = await chatApi.getConversationFiles(conversationId)
        if (!mounted) return
  ...
  const presenceUsername = variant === "direct" ? directUsername : undefined

  usePresenceRealtime((payload) => {
    if (!conversationId || payload.userId === myUserId || !presenceUsername) return
    void chatApi.getUserByUsername(presenceUsername).then((peer) => {
  ...
  }, [conversationId, socket, syncMessages, presenceUsername])
  ...
  const peerLabel = directDisplayName ?? directUsername ?? ""
      ...
      <header className="flex h-14 shrink-0 items-center gap-2 border-b px-4">
        ...
        {variant === "team" ? (
          <div className="flex min-w-0 flex-col">
            <h1 className="truncate font-semibold">{teamTitle ?? "Channel"}</h1>
            {teamSubtitle ? (
              <span className="truncate text-xs text-muted-foreground">{teamSubtitle}</span>
            ) : null}
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <span
              className={`size-2 rounded-full ${peerStatus === "active" ? "bg-emerald-500" : "bg-gray-400"}`}
            />
            <h1 className="font-semibold">@{peerLabel}</h1>
      ...
      <div className="flex min-h-0 flex-1 flex-col bg-muted/30">
        ...
        <div ref={listRef} className="flex-1 space-y-2 overflow-y-auto p-4">
          ...
          {messages.map((message) => {
            ...
            const teamSenderLine =
              variant === "team"
                ? `${message.senderUsername?.trim() || "Unknown"}${isMine ? " (you)" : ""}`
                : null
            return (
              <div key={message._id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                <div className="flex max-w-[78%] flex-col">
                  ...
                  <div
                    ...
                    } ${isAudioAttachment ? "w-[min(22rem,82vw)]" : ""}`}
                  >
                    {isEditing ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          >
                            <remixIconReact.RiCloseLine className="size-3.5" />
                            Cancel
                          </button>
                          <button
                            type="button"
                            ...
                            className="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs"
                          >
                            <remixIconReact.RiSaveLine className="size-3.5" />
                            Save
                          </button>
                        </div>
                      </div>
                    ...
                    {message.fileUrl ? (
                      <div className="mt-2">
                        ...
                        {attachmentKind(message.fileUrl) === "video" ? (
                          <video controls className="max-h-80 rounded-lg" src={message.fileUrl} />
                        ) : null}
                        ...
                        {attachmentKind(message.fileUrl) === "file" ? (
                          <div
                            className={`flex items-center justify-between gap-3 rounded-lg p-3 ${isMine ? "bg-primary-foreground/20" : "bg-muted"}`}
                          >
                            <div className="flex min-w-0 items-center gap-2">
                              <remixIconReact.RiFileTextLine className="size-5 shrink-0" />
                              <span className="truncate text-xs">
                                {filesByUrl[message.fileUrl]?.fileName ??
                                  resolveAttachmentName(message.fileUrl)}
                              </span>
                            </div>
                            <a
                              ...
                              className="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs"
                            >
                              <remixIconReact.RiFileDownloadLine className="size-4" />
                              Download
                            </a>
                          </div>
                        ) : null}
                  ...
                  <div
                    className={`mt-1 flex items-center gap-2 text-[11px] text-muted-foreground ${isMine ? "justify-end" : "justify-start"}`}
                  >
                    ...
                    {isMine && !isEditing ? (
                      <dropdownMenu.DropdownMenu>
                        <dropdownMenu.DropdownMenuTrigger asChild>
                          ...
                          >
                            <remixIconReact.RiMore2Fill className="size-3.5" />
                          </button>
                        </dropdownMenu.DropdownMenuTrigger>
                        <dropdownMenu.DropdownMenuContent align={isMine ? "end" : "start"} className="w-32">
                          <dropdownMenu.DropdownMenuItem onClick={() => handleStartEdit(message)}>
                            <remixIconReact.RiEdit2Line className="size-4" />
                            Edit
                          ...
                          >
                            <remixIconReact.RiDeleteBinLine className="size-4" />
                            Delete
                          </dropdownMenu.DropdownMenuItem>
        ...
        {!threadLoading && typingUsers.length > 0 ? (
        ...
        <form onSubmit={handleSend} className="border-t bg-background p-3">
          <div className="rounded-2xl border bg-background">
            ...
            <div className="flex items-center justify-between border-t px-2 py-2">
              ...
              >
                <remixIconReact.RiArrowUpLine className="size-4" />
              </button>
            </div>
          </div>
        </form>
